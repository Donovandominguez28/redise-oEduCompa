...
<head>
  <style>
    /* Estilo predeterminado sin margen superior */
    body header {
      margin-top: 0; /* Sin margen superior inicialmente */
    }

    /* Ocultar el icono y el tooltip */
    #goog-gt-tt, .goog-te-gadget-icon {
      display: none !important;
    }

    /* Estilizar la barra de Google Translate */
    .goog-te-banner-frame {
      height: 40px !important; /* Ajusta la altura según tus necesidades */
      width: 100%;
      z-index: 1000; /* Mantén un z-index positivo */
...
// Detectar cuando el Google Translate está activo y ajustar el margen superior
function adjustMarginForTranslate() {
  var bannerFrame = document.querySelector('.goog-te-banner-frame');
  var header = document.querySelector('header');

  if (bannerFrame && bannerFrame.style.display !== 'none') {
    header.style.marginTop = '40px'; // Aplica el margen superior cuando la barra está visible
  } else {
    header.style.marginTop = '0'; // Quita el margen si la barra no está activa
  }
}

// Revisar continuamente si el Google Translate está activo
setInterval(adjustMarginForTranslate, 500);
</script>
